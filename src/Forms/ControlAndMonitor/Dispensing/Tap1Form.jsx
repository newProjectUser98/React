        ...
        const fetchData = () => {
            ...
            axios.post("/topicapi/updated_disp_tap1/", newData).then((resp) => {
                ...
                if (resp.data[0].data.data_set.p1 !== 0) {
                    localStorageDataTap1.p1 = resp.data[0].data.data_set.p1;
                }
                        ...
                        return (
                            <Form autoComplete='off'>
                                ...
                                <div className="flex items-center py-3 flex-wrap">
                                    ...
                                    <Field disabled={!editSetting} type="number" required name="p1" id="p1" value={p1} onChange={(e) => setP1(e.target.value)} className="my-2 p-3 border rounded-md w-52 outline-none font-medium text-sm leading-5" placeholder="Pulse1" />
                                </div>
                                <div className="flex items-center py-3 flex-wrap">
                                    <div className="rounded-full bg-green-400 w-3 h-3 mx-2"></div>
                                    <p className='w-40 my-2'>Pulse2</p>
                                    <Field disabled={!editSetting} type="number" required name="p2" id="p2" value={p2} onChange={(e) => setP2(e.target.value)} className="my-2 p-3 border rounded-md w-52 outline-none font-medium text-sm leading-5" placeholder="Pulse2" />
                                </div>
                                <div className="flex items-center py-3 flex-wrap">
                                    <div className="rounded-full bg-green-400 w-3 h-3 mx-2"></div>
                                    <p className='w-40 my-2'>Pulse3</p>
                                    <Field disabled={!editSetting} type="number" required name="p3" id="p3" value={p3} onChange={(e) => setP3(e.target.value)} className="my-2 p-3 border rounded-md w-52 outline-none font-medium text-sm leading-5" placeholder="Pulse3" />
                                </div>
                                <div className="flex items-center py-3 flex-wrap">
                                    <div className="rounded-full bg-green-400 w-3 h-3 mx-2"></div>
                                    <p className='w-40 my-2'>Pulse4</p>
                                    <Field disabled={!editSetting} type="number" required name="p4" id="p4" value={p4} onChange={(e) => setP4(e.target.value)} className="my-2 p-3 border rounded-md w-52 outline-none font-medium text-sm leading-5" placeholder="Pulse4" />
                                </div>
                                {
                                    editSetting &&
